...
class DashboardService {
            ...
            db_1.default.query(query, (error, result) => {
                ...
                const promises = result.map((row) => {
                    return new Promise((resolve, reject) => {
                        ...
                        db_1.default.query(tcGroupQuery, [row.r_tc_name], (error, tcGroupResult) => {
                            if (error) {
                                return reject(error);
                            }
    ...
    tcResultPolicy() {
        return new Promise((resolve, reject) => {
            ...
            db_1.default.query(sessionQuery, (error, sessions) => {
                if (error) {
                    return reject(error);
                }
                const promises = sessions.map(session => {
                    return new Promise((resolve, reject) => {
                        ...
                        db_1.default.query(tcResultQuery, [session.s_id], (error, tcResults) => {
                            if (error) {
                                return reject(error);
                            }
                            const policyQuery = "SELECT tc_name, tc_group FROM tc_policy WHERE p_name = ?";
                            db_1.default.query(policyQuery, [session.p_name], (error, policyResults) => {
                                if (error) {
                                    return reject(error);
                                }
                                // 그룹별로 요약 결과를 저장할 객체
                ...
                Promise.all(promises)
                    ...
                    results.forEach(groupSummary => {
                        for (const [group, summary] of Object.entries(groupSummary)) {
    ...
    failTC() {
        return new Promise((resolve, reject) => {
            ...
            db_1.default.query(tcResultQuery, (error, result) => {
                if (error) {
                    reject(error);
                }
                //Promise 배열 생성
    ...
    failSessionCount() {
        return new Promise((resolve, reject) => {
            ...
            db_1.default.query(sessionQuery, (error, result) => {
                if (error) {
                    reject(error);
                }
                ...
                const promises = result.map((row) => {
                    return new Promise((resolve, reject) => {
                        ...
                        db_1.default.query(contQuery, [row.s_id], (error, contResult) => {
                            if (error) {
                                return reject(error);
                            }
                            let hasFail = false;
            ...
            db_1.default.query(sessionQuery, (error, result) => {
                ...
                const promises = result.map((row) => {
                    return new Promise((resolve, reject) => {
                        ...
                        db_1.default.query(tcGroupQuery, [row.p_name], (error, tcGroupResult) => {
                            if (error) {
                                return reject(error);
                            }
                ...
                Promise.all(promises)
                    ...
                    const combinedResults = finalResults.reduce((acc, curr) => {
                        const existing = acc.find((item) => item.tc_group === curr.tc_group);
                        if (existing) {
