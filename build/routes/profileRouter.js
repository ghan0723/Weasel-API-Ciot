...
router.get("/edit/:username", (req, res) => {
    ...
    profileService
        ...
        log_1.weasel.log(username, req.socket.remoteAddress, `[Info] The user '${username}' accesses the Edit Personal information menu.`);
    ...
    profileService
        ...
        if (result1[0].privilege !== 1) {
            ...
            userService
                ...
                .catch(() => {
                log_1.weasel.error(oldname, req.socket.remoteAddress, `[Error] The user '${oldname}' encountered an error while executing the query to query the database for the username to be changed in Edit Profile.`);
            ...
            userService
                ...
                .catch(() => {
                log_1.weasel.error(oldname, req.socket.remoteAddress, `[Warn] The user '${oldname}' encountered an error while executing a query to query the database for the username to change in Edit Personal Information.`);
                // weasel.error(oldname,req.socket.remoteAddress, "본인정보수정에서 변경할 사용자명을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
                res.status(401).send({ error: result1.message });
            });
        }
