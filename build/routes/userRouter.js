                ...
                if (result1.exists) {
                    ...
                    userService.checkIpRange(user.range, IpRange).then((result3) => {
                        if (result3.inRange) {
                            ...
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, "There was an error executing a query to the database to look up the rating and IP band of the currently logged in user.");
                            // weasel.log(user.cookie, req.socket.remoteAddress, "현재 로그인한 사용자의 등급과 IP 대역을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
                ...
                if (result1[0].privilege !== 1) {
                    ...
                    userService
                        ...
                        if (result3.inRange) {
                            ...
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, "An error occurred while executing a query that queries the database to see if the IP band you entered exceeds the IP band of the currently logged in user.");
                            // weasel.log(user.cookie, req.socket.remoteAddress, "입력한 IP 대역이 현재 로그인한 사용자의 IP 대역을 초과하는지 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
                            res.status(401).send({ error: result3.message });
                        }
                    });
                }
