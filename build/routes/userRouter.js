...
router.post("/login", (req, res) => {
    ...
    userService
        ...
        .then((user) => {
        if (user.length === 0) {
            log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' The username doesn't exist.`);
            ...
            userService
                ...
                if (result[0].privilege === 1) {
                    ...
                    settingService
                        ...
                        .then((cookieTime) => {
                        if (passwd !== decPasswd) {
                            log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' The username or password is incorrect.`);
                            ...
                            userService
                                ...
                                if (((_a = popup[0]) === null || _a === void 0 ? void 0 : _a.count) > 0) {
                                    ...
                                    log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [UserService] - User '${username}' Check the contents of the Sign in successfully popup.`);
                                    ...
                                    log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [UserService] - User '${username}' successfully logged in.`);
                                ...
                                .catch((error5) => {
                                log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing the query that queries the popup to the database.`);
                        ...
                        .catch((error2) => {
                        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing a query to look up the server's Guitime in the database.`);
                    ...
                    if (((_a = user[0]) === null || _a === void 0 ? void 0 : _a.enabled) !== 1) {
                        log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' The user's status is not enabled.`);
                        ...
                        settingService
                            ...
                            userService
                                ...
                                if (passwd !== decPasswd) {
                                    userService
                                        ...
                                        if (user[0].fail_count + 1 >= 5) {
                                            log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' temporarily inaccessible because password incorrectly more than five times.`);
                                            // weasel.log(username,req.socket.remoteAddress,"비밀번호를 5회 이상 잘못 입력하여 계정이 일시적으로 접근 불가 상태로 변경됩니다.");
                                        }
                                        else {
                                            log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' Incorrect password ${user[0].fail_count + 1} times.`);
                                        ...
                                        .catch((enableError) => {
                                        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing a query to initialize the number of failed password entry attempts.`);
                                    ...
                                    if (!freq) {
                                        ...
                                        userService
                                            ...
                                            .catch((error5) => {
                                            log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing a query that queries the database for the number of failed password attempts.`);
                                        ...
                                        log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [UserService] - User '${username}' overdue for a password change. Go to the Change password screen`);
                                ...
                                .catch((error3) => {
                                log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing a query that queries the database for password change intervals.`);
                ...
                .catch((error) => {
                log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while running the query to query the database for the rating of the ID.`);
        ...
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing the query to look up the entered ID in the database.`);
    ...
    userService
        ...
        if (result[0].privilege !== 1) {
            userService
                ...
                .then((result1) => {
                if (result1.exists) {
                    log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[WARN] [UserService] - User '${user.cookie}' username is a duplicate and cannot be created.`);
                        ...
                        if (result3.inRange) {
                            ...
                            userService
                                ...
                                userService
                                    ...
                                    .then((result4) => {
                                    log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[INFO] [UserService] - User '${user.cookie}' Successfully created a new user account.`);
                                    // weasel.log(user.cookie, req.socket.remoteAddress, "새로운 사용자 계정 생성에 성공하였습니다.");
                                    res.send(result4.message);
                                })
                                    .catch((error) => {
                                    log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' An error occurred while executing a query to add a new user account to the database.`);
                                    // weasel.error(user.cookie, req.socket.remoteAddress, "새로운 사용자 계정을 데이터베이스에 추가하는 쿼리 실행 중 오류가 발생하였습니다.");
                                    res.status(500).send(error);
                                });
                            })
                                .catch((error) => {
                                log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' There was an error running a query to the database to see how often the currently logged in user has changed their password.`);
                                // weasel.error(user.cookie, req.socket.remoteAddress, "현재 로그인한 사용자의 비밀번호 변경 주기를 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
                                res.status(500).send(error);
                            });
                        }
                        else {
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[WARN] [UserService] - User '${user.cookie}' can't create an account that exceeds the IP band of the user currently logged in.`);
                ...
                .catch((error) => {
                log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' An error occurred while executing a query to look up a new username in the database.`);
            ...
            userService.checkUsername(newUser.username).then((result5) => {
                if (result5.exists) {
                    log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[WARN] [UserService] - User '${user.cookie}' The username is a duplicate and cannot be created.`);
                    // weasel.log(user.cookie, req.socket.remoteAddress, "입력한 사용자명이 중복되어 생성할 수 없습니다.");
                    res.status(401).send({ error: '입력한 사용자명이 중복되어 생성할 수 없습니다.' });
                }
                else {
        ...
        .catch(() => {
        log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' There was an error executing a query to the database to look up the rating and IP band of the currently logged in user.`);
        ...
        userService
            ...
            if (result[0].privilege !== 1) {
                userService
                    ...
                    .then((result2) => {
                    log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [UserService] - User '${username}' Successfully deleted the user.`);
                    // weasel.log(username, req.socket.remoteAddress, "사용자 삭제를 성공하였습니다.");
                    res.status(200).send(result2);
                })
                    .catch((error2) => {
                    log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' Failed to navigate to the Manage Users menu after deleting a user account.`);
            ...
            .catch((error) => {
            log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' There was an error executing a query to the database to look up the rating and IP band of the currently logged in user.`);
            // weasel.error(username, req.socket.remoteAddress, "현재 로그인한 사용자의 등급과 IP 대역을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
            res.status(500).send("Internal Server Error");
        });
    })
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while executing a query to delete user accounts to the database.`);
...
router.get("/modify/:username", (req, res) => {
    ...
    userService
        ...
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' Failed to navigate to the Edit User menu.`);
...
router.post("/update/:username", (req, res) => {
    ...
    userService
        ...
        userService
            ...
            .then((result) => {
            if (result.exists) {
                log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[WARN] [UserService] - User '${user.cookie}' The username is a duplicate and can't be changed.`);
                ...
                if (result1[0].privilege !== 1) {
                    ...
                    userService_1.default
                        ...
                        if (result3.inRange) {
                            ...
                            userService.getPwdByUsername(oldname).then((result) => {
                                ...
                                if (decOldPwd === user.passwd) {
                                    ...
                                    userService
                                        ...
                                        .then((result4) => {
                                        log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[INFO] [UserService] - User '${user.cookie}' The user modification was successful.`);
                                        // weasel.log(user.cookie, req.socket.remoteAddress, "사용자 수정을 성공하였습니다.");
                                        res.send(result4.message);
                                    })
                                        .catch((error) => {
                                        log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' An error occurred while executing a query to change user accounts in the database.`);
                                    ...
                                    userService
                                        ...
                                        userService
                                            ...
                                            .catch((error) => {
                                            log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' Renewing the password change cycle for the changed user account failed.`);
                            ...
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[WARN] [UserService] - User '${user.cookie}' can't change to an account that exceeds the IP band of the user currently logged in.`);
                                ...
                                if (decOldPwd === user.passwd) {
                                    ...
                                    userService
                                        ...
                                        userService
                                            ...
                                            .catch((error) => {
                                            log_1.weasel.error(user.username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' Renewing the password change cycle for the changed user account failed.`);
            ...
            .catch((error) => {
            log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[ERROR] [UserService] - User '${user.cookie}' An error occurred while executing the query that queries the database for the username to change.`);
...
router.post("/pwd", (req, res) => {
    ...
    userService
        ...
        if (user.oldPwd !== decOldPwd) {
            log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' the password incorrectly before changing it.`);
            ...
            if (user.newPwd !== user.oldPwd) {
                userService
                    ...
                    .then((result2) => {
                    log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [UserService] - User '${username}' successfully changed account password.`);
                    // weasel.log(username,req.socket.remoteAddress,"계정 비밀번호 변경을 성공했습니다.");
                    res.status(200).send(result2);
                })
                    .catch((error) => {
                    log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' Password cycle renewal failed.`);
                    // weasel.error(username,req.socket.remoteAddress,"비밀번호 주기를 새로 갱신하는데 실패하였습니다.");
                    res.status(500).send("Internal Server Error");
                });
            }
            else {
                log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [UserService] - User '${username}' The new password matches the existing password.`);
        ...
        .catch((error2) => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [UserService] - User '${username}' An error occurred while running a query to the database for the pre-change password.`);
