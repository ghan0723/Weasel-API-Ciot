...
router.post("/login", (req, res) => {
    ...
    userService
        ...
        .then((user) => {
        if (user.length === 0) {
            log_1.weasel.log(username, req.socket.remoteAddress, "The username you entered doesn't exist.");
            ...
            userService
                ...
                if (result[0].privilege === 1) {
                    ...
                    settingService
                        ...
                        .then((cookieTime) => {
                        if (passwd !== decPasswd) {
                            log_1.weasel.log(username, req.socket.remoteAddress, "The username or password is incorrect.");
                            ...
                            userService
                                ...
                                if (((_a = popup[0]) === null || _a === void 0 ? void 0 : _a.count) > 0) {
                                    ...
                                    log_1.weasel.log(username, req.socket.remoteAddress, "Check the contents of the Sign in successfully popup.");
                                    ...
                                    log_1.weasel.log(username, req.socket.remoteAddress, "You have successfully logged in.");
                                ...
                                .catch((error5) => {
                                log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing the query that queries the popup to the database.");
                        ...
                        .catch((error2) => {
                        log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing a query to look up the server's Guitime in the database.");
                    ...
                    if (((_a = user[0]) === null || _a === void 0 ? void 0 : _a.enabled) !== 1) {
                        ...
                        settingService
                            ...
                            userService
                                ...
                                if (passwd !== decPasswd) {
                                    userService
                                        ...
                                        if (user[0].fail_count + 1 >= 5) {
                                            log_1.weasel.log(username, req.socket.remoteAddress, "Your account is temporarily inaccessible because you've entered your password incorrectly more than five times.");
                                            // weasel.log(username,req.socket.remoteAddress,"비밀번호를 5회 이상 잘못 입력하여 계정이 일시적으로 접근 불가 상태로 변경됩니다.");
                                        }
                                        else {
                                            log_1.weasel.log(username, req.socket.remoteAddress, `Incorrect password ${user[0].fail_count + 1} times.`);
                                        ...
                                        .catch((enableError) => {
                                        log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing a query to initialize the number of failed password entry attempts.");
                                    ...
                                    if (!freq) {
                                        ...
                                        userService
                                            ...
                                            .catch((error5) => {
                                            log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing a query that queries the database for the number of failed password attempts.");
                                        ...
                                        log_1.weasel.log(username, req.socket.remoteAddress, "You're overdue for a password change. Go to the Change password screen");
                                ...
                                .catch((error3) => {
                                log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing a query that queries the database for password change intervals.");
                ...
                .catch((error) => {
                log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while running the query to query the database for the rating of the ID you entered.");
        ...
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing the query to look up the entered ID in the database.");
    ...
    userService
        ...
        if (result[0].privilege !== 1) {
            userService
                        ...
                        if (result3.inRange) {
                            ...
                            userService
                                ...
                                userService
                                    ...
                                    .then((result4) => {
                                    log_1.weasel.log(user.cookie, req.socket.remoteAddress, "Successfully created a new user account.");
                                    // weasel.log(user.cookie, req.socket.remoteAddress, "새로운 사용자 계정 생성에 성공하였습니다.");
                                    res.send(result4.message);
                                })
                                    .catch((error) => {
                                    log_1.weasel.error(user.cookie, req.socket.remoteAddress, "An error occurred while executing a query to add a new user account to the database.");
                                    // weasel.error(user.cookie, req.socket.remoteAddress, "새로운 사용자 계정을 데이터베이스에 추가하는 쿼리 실행 중 오류가 발생하였습니다.");
                                    res.status(500).send(error);
                                });
                            })
                                .catch((error) => {
                                log_1.weasel.error(user.cookie, req.socket.remoteAddress, "There was an error running a query to the database to see how often the currently logged in user has changed their password.");
                                // weasel.error(user.cookie, req.socket.remoteAddress, "현재 로그인한 사용자의 비밀번호 변경 주기를 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
                                res.status(500).send(error);
                            });
                        }
                        else {
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, "You can't create an account that exceeds the IP band of the user currently logged in.");
                ...
                .catch((error) => {
                log_1.weasel.error(user.cookie, req.socket.remoteAddress, "An error occurred while executing a query to look up a new username in the database.");
        ...
        .catch(() => {
        log_1.weasel.error(user.cookie, req.socket.remoteAddress, "There was an error executing a query to the database to look up the rating and IP band of the currently logged in user.");
        ...
        userService
            ...
            if (result[0].privilege !== 1) {
                userService
                    ...
                    .catch((error2) => {
                    log_1.weasel.error(username, req.socket.remoteAddress, "Failed to navigate to the Manage Users menu after deleting a user account.");
            ...
            .catch((error) => {
            log_1.weasel.error(username, req.socket.remoteAddress, "There was an error executing a query to the database to look up the rating and IP band of the currently logged in user.");
            // weasel.error(username, req.socket.remoteAddress, "현재 로그인한 사용자의 등급과 IP 대역을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
            res.status(500).send("Internal Server Error");
        });
    })
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, "An error occurred while executing a query to delete user accounts to the database.");
...
router.get("/modify/:username", (req, res) => {
    ...
    userService
        ...
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, "Failed to navigate to the Edit User menu.");
...
router.post("/update/:username", (req, res) => {
    ...
    userService
        ...
        userService
            ...
            .then((result) => {
            if (result.exists) {
                log_1.weasel.log(user.cookie, req.socket.remoteAddress, "The username you entered is a duplicate and can't be changed.");
                ...
                if (result1[0].privilege !== 1) {
                    ...
                    userService_1.default
                        ...
                        if (result3.inRange) {
                            ...
                            userService.getPwdByUsername(oldname).then((result) => {
                                ...
                                if (decOldPwd === user.passwd) {
                                    ...
                                    userService
                                        ...
                                        .then((result4) => {
                                        log_1.weasel.log(user.cookie, req.socket.remoteAddress, "The user modification was successful.");
                                        // weasel.log(user.cookie, req.socket.remoteAddress, "사용자 수정을 성공하였습니다.");
                                        res.send(result4.message);
                                    })
                                        .catch((error) => {
                                        log_1.weasel.error(user.cookie, req.socket.remoteAddress, "An error occurred while executing a query to change user accounts in the database.");
                                    ...
                                    userService
                                        ...
                                        userService
                                            ...
                                            .catch((error) => {
                                            log_1.weasel.error(user.cookie, req.socket.remoteAddress, "Renewing the password change cycle for the changed user account failed.");
                            ...
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, "You can't change to an account that exceeds the IP band of the user currently logged in.");
                                ...
                                if (decOldPwd === user.passwd) {
                                    ...
                                    userService
                                        ...
                                        userService
                                            ...
                                            .catch((error) => {
                                            log_1.weasel.error(user.username, req.socket.remoteAddress, "Renewing the password change cycle for the changed user account failed.");
            ...
            .catch((error) => {
            log_1.weasel.error(user.cookie, req.socket.remoteAddress, "An error occurred while executing the query that queries the database for the username to change.");
...
router.post("/pwd", (req, res) => {
    ...
    userService
        ...
        if (user.oldPwd !== decOldPwd) {
            log_1.weasel.log(username, req.socket.remoteAddress, "You entered the password incorrectly before changing it.");
            ...
            if (user.newPwd !== user.oldPwd) {
                userService
                    ...
                    .then((result2) => {
                    log_1.weasel.log(username, req.socket.remoteAddress, "You successfully changed your account password.");
                    // weasel.log(username,req.socket.remoteAddress,"계정 비밀번호 변경을 성공했습니다.");
                    res.status(200).send(result2);
                })
                    .catch((error) => {
                    log_1.weasel.error(username, req.socket.remoteAddress, "Password cycle renewal failed.");
