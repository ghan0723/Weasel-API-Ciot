...
router.post("/login", (req, res) => {
    ...
    userService
        ...
        .then((user) => {
        if (user.length === 0) {
            log_1.weasel.log(username, req.socket.remoteAddress, `[Warn] The username '${username}' doesn't existed.`);
            ...
            userService
                ...
                if (result[0].privilege === 1) {
                    ...
                    settingService
                        ...
                        .then((cookieTime) => {
                        if (passwd !== decPasswd) {
                            log_1.weasel.log(username, req.socket.remoteAddress, `[Warn] The username '${username}' is not the corrected username or password.`);
                            ...
                            userService
                                ...
                                if (((_a = popup[0]) === null || _a === void 0 ? void 0 : _a.count) > 0) {
                                    ...
                                    log_1.weasel.log(username, req.socket.remoteAddress, `[Info] The user, '${username}' checked the contents of the Sign in successfully popup.`);
                    ...
                    if (((_a = user[0]) === null || _a === void 0 ? void 0 : _a.enabled) !== 1) {
                        log_1.weasel.log(username, req.socket.remoteAddress, `[Warn] The user, '${username}'s status is not activated.`);
                                ...
                                if (passwd !== decPasswd) {
                                    userService
                                        ...
                                        if (user[0].fail_count + 1 >= 5) {
                                            ...
                                            log_1.weasel.log(username, req.socket.remoteAddress, `[Warn] The user, '${username}' incorrected password ${user[0].fail_count + 1} times.`);
                                    ...
                                    if (!freq) {
                                        ...
                                        log_1.weasel.log(username, req.socket.remoteAddress, `[Info] The user, '${username}' overdued for a password change. Go to the Change password screen`);
...
router.post("/update/:username", (req, res) => {
    ...
    userService
        ...
        userService
            ...
            .then((result) => {
            if (result.exists) {
                log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' cannot be changed because The user,name entered is a duplicate.`);
                ...
                if (result1[0].privilege !== 1) {
                    ...
                    userService_1.default
                        ...
                        if (result3.inRange) {
                            ...
                            userService.getPwdByUsername(oldname).then((result) => {
                                ...
                                if (decOldPwd === user.passwd) {
                                    ...
                                    userService
                                        ...
                                        .then((result4) => {
                                        log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[Info] The user, '${user.cookie}' modification was successed.`);
                            ...
                            log_1.weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' can't changed to an account that exceeds the IP band of The user, currently logged in.`);
            ...
            .catch((error) => {
            log_1.weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while executing the query to query the database for The user,name to change.`);
...
router.post("/pwd", (req, res) => {
    ...
    userService
        ...
        if (user.oldPwd !== decOldPwd) {
            log_1.weasel.log(username, req.socket.remoteAddress, `[Warn] The user, '${username}' the password incorrected before changing it.`);
