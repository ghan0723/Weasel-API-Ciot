...
router.post("/server", (req, res) => {
    ...
    settingService
        ...
        settingService.modServerSetting(server)
            ...
            log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' Failed to navigate to the Server Settings menu.`);
        ...
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' An error occurred while executing a query that changes server settings to the database.`);
        res.status(500).send("update 하다가 에러났어요");
    });
});
    ...
    userService.getPrivilege(username)
        ...
        settingService
            ...
            if (result1[0].privilege === 3) {
                log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [SettingService] - User '${username}' This account does not have access to server settings.`);
                // weasel.log(username, req.socket.remoteAddress, "서버 설정을 이용할 수 없는 계정입니다.");
                res.send(newResult);
            }
            else {
                log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [SettingService] - User '${username}' directed to the Server Settings menu.`);
...
router.post("/agent", (req, res) => {
    ...
    settingService
        ...
        settingService.modAgentSetting(checkAgent)
            ...
            log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' Navigating to the agent settings menu failed.`);
            // weasel.error(username, req.socket.remoteAddress, "에이전트 설정 메뉴로 이동에 실패하였습니다.");
            res.status(500).send("agent setting post 하다가 에러났어요");
        });
    })
        .catch(() => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' An error occurred while executing a query to change agent settings to the database.`);
    ...
    userService.getPrivilege(username)
        ...
        settingService
            ...
            settingService
                ...
                .then((result2) => {
                if (result1[0].privilege === 3) {
                    log_1.weasel.log(username, req.socket.remoteAddress, `[WARN] [SettingService] - User '${username}' Agent settings are not available for this account.`);
                    // weasel.log(username, req.socket.remoteAddress, "에이전트 설정을 이용할 수 없는 계정입니다.");
                    res.send([result, result2, result1]);
                }
                else {
                    log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [SettingService] - User '${username}' accesses the Agent settings menu.`);
    ...
    settingService.checkProcessAccuracy(newProcName)
        ...
        if (((_a = result[0]) === null || _a === void 0 ? void 0 : _a.result) === 0) {
            settingService
                ...
                .then((addResult) => {
                log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [SettingService] - User '${username}' Added ${newProcName} to the reconnaissance process.`);
                // weasel.log(username, req.socket.remoteAddress, `${newProcName}을 정탐 프로세스에 추가하였습니다.`);
                res.send(addResult);
            })
                .catch((error) => {
                log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' Adding reconnaissance process ${newProcName} failed.`);
        ...
        .catch(error => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' An error occurred while executing a query to query the database for a new reconnaissance process to add.`);
...
router.post("/delete", (req, res) => {
    ...
    settingService
        ...
        .then((result) => {
        log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [SettingService] - User '${username}' Deleted ${procName} from the reconnaissance process.`);
        // weasel.log(username, req.socket.remoteAddress, `${procName}을 정탐 프로세스에서 삭제하였습니다.`);
        res.send(result);
    })
        .catch((error) => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' Failed to Delete ProcessAccuracy`);
...
router.post("/updateFile", (req, res) => {
    ...
    settingService
        ...
        .then(() => {
        log_1.weasel.log(username, req.socket.remoteAddress, `[INFO] [SettingService] - User '${username}' The weasel agent has been updated with ${updateFile}.`);
        // weasel.log(username, req.socket.remoteAddress, `${updateFile}로 weasel 에이전트가 업데이트 되었습니다.`);
        res.send(updateFile);
    })
        .catch(() => {
        log_1.weasel.error(username, req.socket.remoteAddress, `[ERROR] [SettingService] - User '${username}' The weasel agent failed to update to ${updateFile}.`);
