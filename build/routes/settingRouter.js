...
router.post("/server", (req, res) => {
    ...
    settingService
        ...
        .then((result) => {
        log_1.weasel.log(username, "172.31.168.112", "Success to Update Server Setting [Server]");
        res.send("업데이트 성공했습니다.");
    })
        .catch((error) => {
        log_1.weasel.error(username, "172.31.168.112", "Failed to Update Server Setting [Server]");
...
router.get("/servers", (req, res) => {
    ...
    settingService
        ...
        log_1.weasel.log(username, "172.31.168.112", "Success to Get Server Information [Server]");
        res.send(newResult);
    })
        .catch((error) => {
        log_1.weasel.error(username, "172.31.168.112", "Failed to Get Server Information [Server]");
...
router.post("/agent", (req, res) => {
    ...
    settingService
        ...
        .then((result) => {
        log_1.weasel.log(username, "172.31.168.112", "Success to Update Agent Setting [Agent]");
        res.send(result);
    })
        .catch((error) => {
        // console.error("agent setting post 에러 : ", error);
        log_1.weasel.error(username, "172.31.168.112", "Failed to Update Agent Setting [Agent]");
...
router.get("/agents", (req, res) => {
    ...
    settingService
        .getAgentSetting()
        .then((result) => {
        log_1.weasel.log(username, "172.31.168.112", "Success to Get Agent Information [Agent]");
        res.send(result);
    })
        .catch((error) => {
        log_1.weasel.error(username, "172.31.168.112", "Failed to Get Agent Information [Agent]");
