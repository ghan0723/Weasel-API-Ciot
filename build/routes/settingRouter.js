    ...
    userService.getPrivilege(username)
        ...
        settingService
            ...
            if (result1[0].privilege === 3) {
                log_1.weasel.log(username, req.socket.remoteAddress, `[Warn] The user '${username}' does not have access to Server Settings menu.`);
                // weasel.log(username, req.socket.remoteAddress, "서버 설정을 이용할 수 없는 계정입니다.");
                res.send(newResult);
            }
            else {
                log_1.weasel.log(username, req.socket.remoteAddress, `[Info] The user '${username}' directed to the Server Settings menu.`);
        ...
        settingService
            ...
            settingService
                ...
                if (result1[0].privilege === 3) {
                    ...
                    log_1.weasel.log(username, req.socket.remoteAddress, `[Info] The user '${username}' accesses the Agent Settings Menu.`);
