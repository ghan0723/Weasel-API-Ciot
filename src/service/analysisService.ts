...
class AnalysisService {
  ...
  analyzePatterns(detectFiles: any, keywords : any): { [pcGuid: string]: {keyword : number, pattern : number} } {
    const patternsResult: { [pcGuid: string]: {keyword : number, pattern : number} } = {};
  ...
  riskScoring(startDate:any, endDate:any, keywords:any):Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      if (matchResult) {
        ...
        let patternsResult:{ [pcGuid: string]: {keyword : number, pattern : number} } = {};
...
calculateProgress(a: any, b: any, c: any, d:any): number {
