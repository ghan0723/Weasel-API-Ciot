...
class Average {
  analyzeLeaks(detectFiles: any): { leakEventsByAllHour: { [key: number]: number }, averageEventsPerHour: number, anomalyHours: number[] } {
    // 24시간 배열을 0으로 초기화
    const leakEventsByAllHour: { [key: number]: number } = Array.from({ length: 24 }, (_, i) => ({ [i]: 0 }))
