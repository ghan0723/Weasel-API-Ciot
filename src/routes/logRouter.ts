...
router.get("/dashboard", (req: Request, res: Response) => {
  ...
  if (typeof username !== "string") {
    weasel.error(username, req.ip, `[Error] The user, '${username}' failed to navigate to the Dashboard menu.`);
...
router.get("/tables", (req: Request, res: Response) => {
  const username = req.query.username;
  if (typeof username !== "string") {
    weasel.error(username, req.ip,`[Error] The user, '${username}' failed to navigate to the Leak Detection History menu.`);
    // weasel.error(username, req.ip,"유출탐지내역 메뉴로 이동에 실패하였습니다.");
    res.status(500).send("tables log error");
  }
  weasel.log(username,req.ip,`[Info] The user, '${username}' accessed the Leak Detection History menu.`);
...
router.get("/leaked", (req: Request, res: Response) => {
  ...
  .then((result) => {
    if(result[0].privilege === 3){
      weasel.log(username,req.ip,`[Warn] The user, '${username}' is not allowed to use the Watchlist.`);
      // weasel.log(username,req.ip,`관리대상목록을 이용할 수 없는 계정입니다.`);
      res.status(400).send(result[0].privilege);
    } else {
      weasel.log(username,req.ip,`[Info] The user, '${username}' accessed the Watchlist menu.`);
      // weasel.log(username,req.ip,`관리대상 목록 메뉴로 이동하였습니다.`);
      res.send("leaked log success");
    }
  })
  .catch((error) => {
    weasel.error(username, req.ip,  `[Error] The user, '${username}' failed to navigate to the Watchlist menu.`);
...
router.get("/analysis", (req: Request, res: Response) => {
  ...
  if (typeof username !== "string") {
    weasel.error(username, req.ip, `[Error] The user, '${username}' failed to navigate to the Analytics menu.`);
    // weasel.error(username, req.ip, "분석 메뉴로 이동에 실패하였습니다.");
    res.status(500).send("analysis log error");
  }
  weasel.log(username,req.ip,`[Info] The user, '${username}' accessed the Analytics menu.`);
...
router.get("/logout", (req: Request, res: Response) => {
  ...
  if (typeof username !== "string") {
    weasel.error(username, req.ip, `[Error] The user, '${username}' encountered an unexpected error on the server while trying to log out, causing the server to crash.`);
    // weasel.error(username, req.ip, "로그아웃 시도 중에 서버에서 예기치 않은 오류가 발생하여 서버가 중단되었습니다.");
    res.status(500).send("logout log error");
  }
  weasel.log(username,req.ip,`[Info] The user, '${username}' successfully logged out.`);
...
router.get("/file", (req: Request, res: Response) => {
  ...
  logService
    ...
    .then((content) => {
      weasel.log(username, req.ip, `[Info] The user, '${username}' checked the ${file} audit log.`);
      // weasel.log("", req.ip, `${file} 감사 로그를 확인하였습니다.`);
      res.send([content]);
    })
    .catch(() => {
      weasel.error(username, req.ip,`[Error] The user, '${username}' failed to check the audit log.`);
...
router.get("/error/file", (req: Request, res: Response) => {
  ...
  logService
    ...
    .then((content) => {
      weasel.log(username, req.ip, `[Info] The user, '${username}' checked the ${file} error log.`);
      // weasel.log("", req.ip, `${file} 에러 로그를 확인하였습니다.`);
      res.send([content]);
    })
    .catch(() => {
      weasel.error(username, req.ip,`[Error] The user, '${username}' checking the error log for ${ file } failed.`);
...
router.get("/screenshot", (req:Request, res:Response) => {
  ...
  if(fileName !== undefined && fileName !== null){
    weasel.log(username, req.ip, `[Info] The user, '${username}' downloaded a screenshot of ${fileName}.`);
    // weasel.log(username, req.ip, `${fileName}의 스크린샷을 다운로드 하였습니다.`);
  } else {
    weasel.error(username, req.ip, `[Error] The user, '${username}' downloading a screenshot of ${fileName} failed.`);
...
router.get("/download", (req:Request, res:Response) => {
  ...
  const fileName = req.query.fileName;
  if(fileName !== undefined && fileName !== null){
    weasel.log(username, req.ip, `[Info] The user, '${username}' downloaded the file ${fileName}.`);
    // weasel.log(username, req.ip, `${fileName}의 파일을 다운로드 하였습니다.`);
  } else {
    weasel.error(username, req.ip, `[Error] The user, '${username}' failed to download the file ${fileName}.`);
...
router.get("/userList", (req:Request, res:Response) => {
  const username = req.query.username;
  if (typeof username !== "string" ) {
    weasel.error(username, req.ip, `[Error] The user, '${username}' failed to navigate to The user, Management menu.`);
    // weasel.error(username, req.ip, "사용자 관리 메뉴로 이동에 실패하였습니다.");
    res.status(500).send("userList log error");
  } else {
    weasel.log(username, req.ip, `[Info] The user, '${username}' accessed The user, Management menu.`);
