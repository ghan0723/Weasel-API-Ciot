...
router.post("/servers", (req: Request, res: Response) => {
  ...
  .then((result) => {
    ...
    .then(() => {
      ...
      weasel.error(username, req.ip, `[Error] The user, '${username}' failed to navigate to the Server Settings menu.`);
    ...
    .catch(() => {
      weasel.error(username, req.ip, `[Error] The user, '${username}' encountered an error while executing a query to change server settings to the database.`);
  ...
  .then((result1) => {
    ...
    .then((result) => {
      ...
      if(result1[0].privilege === 3){
        weasel.log(username, req.ip, `[Warn] The user, '${username}' does not have accessed to Server Settings menu.`);
        // weasel.log(username, req.ip, "서버 설정을 이용할 수 없는 계정입니다.");
        res.send(newResult);
      } else {
        weasel.log(username, req.ip, `[Info] The user, '${username}' accessed to the Server Settings menu.`);
  ...
  settingService
    ...
    .then(result => {      
      ...
      .then((result) => {
        ...
        weasel.error(username, req.ip, `[Error] The user, '${username}' navigating to the Agent Settings Menu failed.`);
        // weasel.error(username, req.ip, "에이전트 설정 메뉴로 이동에 실패하였습니다.");
        res.status(500).send("agent setting post 하다가 에러났어요");
      });
    })
    .catch(() => {
      weasel.error(username, req.ip, `[Error] The user, '${username}' encountered an error while executing a query to change agent settings to the database.`);
  ...
  .then((result1) => {
    ...
    .then((result) => {
      settingService
        ...
        .then((result2) => {
          if(result1[0].privilege === 3){
            weasel.log(username, req.ip, `[Warn] The user, '${username}' is an account that does not have access to Agent Settings Menu.`);
            // weasel.log(username, req.ip, "에이전트 설정을 이용할 수 없는 계정입니다.");
            res.send([result, result2[0].updateFile, result1]);
          } else {
            weasel.log(username, req.ip, `[Info] The user, '${username}' accessed the Agent Settings Menu.`);
...
router.post("/process", (req: Request, res: Response) => {
  ...
  .then(result => {
    if(result[0]?.result === 0) {
      ...
      .then((addResult) => {
        weasel.log(username, req.ip, `[Info] The user, '${username}' added ${newProcName} to the reconnaissance process.`);
        // weasel.log(username, req.ip, `${newProcName}을 정탐 프로세스에 추가하였습니다.`);
        res.send(addResult);
      })
      .catch((error:any) => {
        weasel.error(username, req.ip, `[Error] The user, '${username}' adding reconnaissance process ${newProcName} failed.`);
  ...
  .catch(error => {
    weasel.error(username, req.ip, `[Error] The user, '${username}' encountered an error while running a query to query the database for a new reconnaissance process to add.`);
...
router.post("/delete", (req: Request, res: Response) => {
  ...
  settingService
    ...
    .then((result) => {
      weasel.log(username, req.ip, `[Info] The user, '${username}' deleted ${procName} from the reconnaissance process.`);
      // weasel.log(username, req.ip, `${procName}을 정탐 프로세스에서 삭제하였습니다.`);
      res.send(result);
    })
    .catch((error) => {
      weasel.error(username, req.ip, `[Error] The user, '${username}' failed to delete ProcessAccuracy`);
...
router.post("/updateFile", (req: Request, res: Response) => {
  ...
  .then(result => {
    ...
    .then(() => {
      weasel.log(username, req.ip, `[Info] The user, '${username}' updated the weasel agent with ${updateFile}.`);
      // weasel.log(username, req.ip, `${updateFile}로 weasel 에이전트가 업데이트 되었습니다.`);
      res.send(updateFile);
    })
    .catch(() => {
      weasel.error(username, req.ip, `[Error] The user, '${username}' failed to update the weasel agent with ${updateFile}.`);
