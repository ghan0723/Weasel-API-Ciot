...
router.post('/rm', (req:Request, res:Response) => {
  ...
  results?.then(() => {     
    getApiDataLogic(contents,0,pageSize,sorting,desc,category,search,username,req,res);
    weasel.log(username, req.socket.remoteAddress, `[INFO] : [WeaselService] - User '${username}' Deleted ${body.length} data in ${contents}.`);
    // weasel.log(username, req.socket.remoteAddress, `${contents}의 ${results.length}개 데이터를 삭제하였습니다.`);
  })
  .catch((error) => {
    weasel.error(username, req.socket.remoteAddress, `[ERROR] : [WeaselService] - User '${username}' Deleted ${body.length} pieces of data in ${contents} fail.`);
