...
router.get("/dwn", async (req: Request, res: Response) => {
  ...
  try {
    ...
    weasel.log(username,req.socket.remoteAddress,`[INFO] [ExcelService] - User '${username}' : Downloaded the Excel file of ${contents}.`);
    // weasel.log(username,req.socket.remoteAddress,`${contents}의 액셀 파일을 다운로드 하였습니다.`);
  } catch (error) {
    weasel.error(username, req.socket.remoteAddress, `[ERROR] [ExcelService] - User '${username}' : Download of the excel file in ${contents} fail.`);
...
router.post("/analytics", async (req: Request, res: Response) => {
  ...
  try {
    ...
    .then(async (result1) => {
      ...
      res.send(excelBuffer);
      weasel.log(username,req.socket.remoteAddress,`[INFO] [ExcelService] - User '${username}' : Downloaded the excel file of the analysis.`);
      // weasel.log(username,req.socket.remoteAddress,"분석의 엑셀 파일을 다운로드 하였습니다.");
    })
  } catch (error) {
    weasel.error(username, req.socket.remoteAddress, `[ERROR] [ExcelService] - User '${username}' : Downloading the analysis excel file failed.`);
