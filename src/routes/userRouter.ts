...
router.post("/login", (req: Request, res: Response) => {
  ...
  userService
    ...
    .then((user) => {
      if (user.length === 0) {
        weasel.log(username, req.socket.remoteAddress, `[Warn] The username '${username}' doesn't existed.`);
          ...
          .then((result) => {
            if (result[0].privilege === 1) {
              ...
              settingService
                ...
                .then((cookieTime) => {
                  if (passwd !== decPasswd) {
                    weasel.log(username,req.socket.remoteAddress,`[Warn] The username '${username}' is not the corrected username or password.`);
                    ...
                    userService
                      ...
                      .then((popup) => {
                        if (popup[0]?.count > 0) {
                          ...
                          weasel.log(username,req.socket.remoteAddress,`[Info] The user, '${username}' checked the contents of the Sign in successfully popup.`);
                          ...
                          weasel.log(username,req.socket.remoteAddress,`[Info] The user, '${username}' successfully logged in.`);
                      ...
                      .catch((error5) => {
                        weasel.error(username,req.socket.remoteAddress,`[Error] The user, '${username}' encountered an error while executing a query that queries the popup to the database.`);
                ...
                .catch((error2) => {
                  weasel.error(username,req.socket.remoteAddress,`[Error] The user, '${username}' encountered an error while executing a query that queried the database for the server's Guitime.`);
              ...
              //관리자 제외 나머지 아이디
              if (user[0]?.enabled !== 1) {
                weasel.log(username,req.socket.remoteAddress,`[Warn] The user, '${username}'s status is not activated.`);
                ...
                settingService
                  ...
                  .then((cookieTime) => {
                    userService
                        ...
                        if (passwd !== decPasswd) {
                          userService
                            ...
                            .then((enabled) => {
                              if(user[0].fail_count + 1 >= 5){
                                weasel.log(username,req.socket.remoteAddress,`[Warn] The user, '${username}' temporarily inaccessible because password incorrectly more than five times.`);
                                // weasel.log(username,req.socket.remoteAddress,"비밀번호를 5회 이상 잘못 입력하여 계정이 일시적으로 접근 불가 상태로 변경됩니다.");
                              } else {
                                weasel.log(username,req.socket.remoteAddress,`[Warn] The user, '${username}' incorrected password ${user[0].fail_count + 1} times.`);
                            ...
                            .catch((enableError) => {
                              weasel.error(username,req.socket.remoteAddress, `[Error] The user, '${username}' encountered an error while running a query to initialize the number of failed password attempts.`);
                          ...
                          if (!freq) {
                            ...
                            userService
                              ...
                              .catch((error5) => {
                                weasel.error(username,req.socket.remoteAddress,`[Error] The user, '${username}'  encountered an error while running a query that queried the database for the number of failed password attempts.`);
                                // weasel.error(username, req.socket.remoteAddress, "비밀번호 입력 실패 횟수를 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생했습니다.");
                                res.status(500).send(error5);
                              });
                          } else {
                            //freq에 의해 비밀번호를 변경해야 한다
                            weasel.log(username,req.socket.remoteAddress,`[Info] The user, '${username}' overdued for a password change. Go to the Change password screen`);
                      ...
                      .catch((error3) => {
                        weasel.error(username,req.socket.remoteAddress,`[Error] The user, '${username}' encountered an error while executing a query that queries the database for password change intervals.`);
...
router.post("/add", (req: Request, res: Response) => {
  ...
  userService
    ...
    .then((result) => {
      ...
      if (result[0].privilege !== 1) {
        userService
          ...
          .then((result1) => {
            if (result1.exists) {
              weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' username is a duplicate and cannot be created.`);
                  ...
                  userService
                    ...
                    .then((result) => {
                      userService
                        ...
                        .then((result4) => {
                          weasel.log(user.cookie, req.socket.remoteAddress, `[Info] The user, '${user.cookie}' successfully created a new user account.`);
                          // weasel.log(user.cookie, req.socket.remoteAddress, "새로운 사용자 계정 생성에 성공하였습니다.");
                          res.send(result4.message);
                        })
                        .catch((error) => {
                          weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while running a query to add a new user account to the database.`);
                          // weasel.error(user.cookie, req.socket.remoteAddress, "새로운 사용자 계정을 데이터베이스에 추가하는 쿼리 실행 중 오류가 발생하였습니다.");
                          res.status(500).send(error);
                        });
                    })
                    .catch((error) => {
                      weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while running a query that queries the database for the password change interval of the currently logged in user.`);
                      // weasel.error(user.cookie, req.socket.remoteAddress, "현재 로그인한 사용자의 비밀번호 변경 주기를 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
                      res.status(500).send(error);
                    });
                } else {
                  weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' can't create an account that exceeds the IP band of The user, currently logged in.`);
          ...
          .catch((error) => {
            weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while running a query to look up a new username in the database.`);
        ...
        userService.checkUsername(newUser.username).then((result5) => {
          if (result5.exists) {
            weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' cannot be created because The username entered is a duplicate.`);
            // weasel.log(user.cookie, req.socket.remoteAddress, "입력한 사용자명이 중복되어 생성할 수 없습니다.");
            res.status(401).send({ error: '입력한 사용자명이 중복되어 생성할 수 없습니다.' });
          } else {
            //관리자 계정 freq
    ...
    .catch(() => {
      weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error executing a query that queries the database for the rating and IP band of the currently logged in user.`);
  ...
  userService
    ...
    .then(() => {
      userService
        ...
        .then((result) => {
          ...
          if (result[0].privilege !== 1) {
            userService
              ...
              .then((result2) => {
                weasel.log(username, req.socket.remoteAddress, `[Info] The user, '${username}' successfully deleted The user.`);
                // weasel.log(username, req.socket.remoteAddress, "사용자 삭제를 성공하였습니다.");
                res.status(200).send(result2);
              })
              .catch((error2) => {
                weasel.error(username, req.socket.remoteAddress, `[Error] The user, '${username}' failed to navigate to the Manage Users Menu after deleting a user account.`);
        ...
        .catch((error) => {
          weasel.error(username, req.socket.remoteAddress, `[Error] The user, '${username}' encountered an error while running a query that queries the database for the rating and IP band of the currently logged in user.`);
          // weasel.error(username, req.socket.remoteAddress, "현재 로그인한 사용자의 등급과 IP 대역을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
          res.status(500).send("Internal Server Error");
        });
    })
    .catch((error) => {
      weasel.error(username, req.socket.remoteAddress, `[Error] The user, '${username}' encountered an error while executing a query to delete The user, account to the database.`);
...
router.get("/modify/:username", (req: Request, res: Response) => {
  ...
  userService
    ...
    .catch((error) => {
      weasel.error(username, req.socket.remoteAddress, `[Error] The user, '${username}' failed to navigate to the Edit User menu.`);
  ...
  userService
    ...
    .then((result1) => {
      userService
        ...
        .then((result) => {
          if (result.exists) {
            weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' cannot be changed because The user,name entered is a duplicate.`);
              ...
              UserService
                ...
                .then((result3) => {
                  if (result3.inRange) {
                    ...
                    userService.getPwdByUsername(oldname).then((result) => {
                      ...
                      if (decOldPwd === user.passwd) {
                        ...
                        userService
                          ...
                          .then((result4) => {
                            weasel.log(user.cookie, req.socket.remoteAddress, `[Info] The user, '${user.cookie}' modification was successed.`);
                            // weasel.log(user.cookie, req.socket.remoteAddress, "사용자 수정을 성공하였습니다.");
                            res.send(result4.message);
                          })
                          .catch((error) => {
                            weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while executing a query to change user accounts in the database.`);
                        ...
                        userService
                          ...
                          .then((result4) => {
                            userService
                              ...
                              .catch((error) => {
                                weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' failed to renew the password change cycle for the changed user account.`);
                    ...
                    weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user, '${user.cookie}' can't changed to an account that exceeds the IP band of The user, currently logged in.`);
                        ...
                        userService
                          ...
                          .then((result4) => {
                            userService
                              ...
                              .catch((error) => {
                                weasel.error(user.username, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' failed to renew the password change cycle for the changed user account.`);
        ...
        .catch((error) => {
          weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while executing the query to query the database for The user,name to change.`);
          // weasel.error(user.cookie, req.socket.remoteAddress, "변경할 사용자명을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
          res.status(500).send("이거는 중복을 검사하는 도중에 발생하는 에러입니다.");
        });
    })
    .catch((error2) => {
      weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user, '${user.cookie}' encountered an error while running a query that queries the database for the rating and IP band of the currently logged in user.`);
      // weasel.error(user.cookie, req.socket.remoteAddress, "현재 로그인한 사용자의 등급과 IP 대역을 데이터베이스에 조회하는 쿼리 실행 중 오류가 발생하였습니다.");
      res.status(500).send(
        "이거는 쿠키 가지고 privilege랑 mngip 가져오는 도중에 발생하는 에러입니다."
      );
  ...
  userService
    ...
    .then((result1) => {
      ...
      if (user.oldPwd !== decOldPwd) {
        weasel.log(username, req.socket.remoteAddress,`[Warn] The user, '${username}' the password incorrected before changing it.`);
        ...
        if (user.newPwd !== user.oldPwd) {
          userService
            ...
            .then((result2) => {
              weasel.log(username, req.socket.remoteAddress, `[Info] The user, '${username}' successfully changed account password.`);
          ...
          weasel.log(username,req.socket.remoteAddress,`[Warn] The user, '${username}' sees that the new password they entered matches their existing password.`);
