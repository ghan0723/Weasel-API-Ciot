...
router.post("/login", (req: Request, res: Response) => {
  ...
  userService
    ...
    .then((user) => {
      if (user.length === 0) {
        weasel.log(username, req.socket.remoteAddress, `[Warn] The username '${username}' doesn't exist.`);
          ...
          .then((result) => {
            if (result[0].privilege === 1) {
              ...
              settingService
                ...
                .then((cookieTime) => {
                  if (passwd !== decPasswd) {
                    weasel.log(username,req.socket.remoteAddress,`[Warn] The username '${username}' is not the correct username or password.`);
                    ...
                    userService
                      ...
                      .then((popup) => {
                        if (popup[0]?.count > 0) {
                          ...
                          weasel.log(username,req.socket.remoteAddress,`[Info] The user '${username}' check the contents of the Sign in successfully popup.`);
              ...
              //관리자 제외 나머지 아이디
              if (user[0]?.enabled !== 1) {
                weasel.log(username,req.socket.remoteAddress,`[Warn] The user '${username}'s status is not active.`);
                      ...
                      .then((freq) => {
                        if (passwd !== decPasswd) {
                          userService
                            ...
                            .then((enabled) => {
                              if(user[0].fail_count + 1 >= 5){
                                ...
                                weasel.log(username,req.socket.remoteAddress,`[Warn] The user '${username}' incorrect password ${user[0].fail_count + 1} times.`);
                          ...
                          if (!freq) {
                            ...
                            weasel.log(username,req.socket.remoteAddress,`[Info] The user '${username}' overdue for a password change. Go to the Change password screen`);
  ...
  userService
    ...
    .then((result1) => {
      userService
        ...
        .then((result) => {
          if (result.exists) {
            weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user '${user.cookie}' cannot be changed because the username entered is a duplicate.`);
              ...
              UserService
                ...
                .then((result3) => {
                  if (result3.inRange) {
                    ...
                    userService.getPwdByUsername(oldname).then((result) => {
                      ...
                      if (decOldPwd === user.passwd) {
                        ...
                        userService
                          ...
                          .then((result4) => {
                            weasel.log(user.cookie, req.socket.remoteAddress, `[Info] The user '${user.cookie}' modification was successful.`);
                    ...
                    weasel.log(user.cookie, req.socket.remoteAddress, `[Warn] The user '${user.cookie}' can't change to an account that exceeds the IP band of the user currently logged in.`);
        ...
        .catch((error) => {
          weasel.error(user.cookie, req.socket.remoteAddress, `[Error] The user '${user.cookie}' encountered an error while executing the query to query the database for the username to change.`);
  ...
  userService
    ...
    .then((result1) => {
      ...
      if (user.oldPwd !== decOldPwd) {
        weasel.log(username, req.socket.remoteAddress,`[Warn] The user '${username}' the password incorrectly before changing it.`);
